...
def telegram_bot(request):
    if request.method == 'POST':
        ...
        return HttpResponseBadRequest('Поганий запит')
    ...
    try:
        ...
        if text == '/register':
            send_message("sendMessage", {
                'chat_id': chat_id,
                'text': 'Будь ласка, надішліть свій контакт:',
                'reply_markup': {
                    'keyboard': [
                        [
                            {
            ...
            send_message("sendMessage", {
                'chat_id': chat_id,
                'text': 'Ви впевнені, що хочете видалити свої дані?',
                'reply_markup': {
                    'keyboard': [
                        [
                            {
            ...
            if success:
                send_message("sendMessage", {
                    'chat_id': chat_id,
                    'text': 'Ваші дані успішно видалені.',
                    'reply_markup': {
                ...
                send_message("sendMessage", {
                    'chat_id': chat_id,
                    'text': 'Не вдалося видалити ваші дані. Будь ласка, спробуйте пізніше.',
                    'reply_markup': {
                        'remove_keyboard': True,
                    }
                })
            ...
            send_message("sendMessage", {
                'chat_id': chat_id,
                'text': 'Дякуємо, що залишаєтеся з нами!',
                'reply_markup': {
                    'remove_keyboard': True,
                }
            })
...
def send_user_qr(chat_id, telegram_id):
    try:
        with connection.cursor() as cursor:
            ...
            if result:
                user_id = result['id']
                qr_data = f"ID користувача: {user_id}"
